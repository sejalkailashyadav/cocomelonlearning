...
<div class="container-fluid mt-10">
 ...
 <td>
                                 ...
                                 <td>
                                    <input type="text" name="reports[{{ $index }}][transit_number]" value="{{ $report->transit_number }}" class="form-control" placeholder="Enter transit_number (optional)" />
                                </td>
                                 <td>
                                    <input type="text" name="reports[{{ $index }}][account_number]" value="{{ $report->account_number }}" class="form-control" placeholder="Enter account_number (optional)" />
                                </td>
                                <!-- Hidden Fields -->
                                <td>
                                    ...
                                    <input class="hidden-field" type="number" step="0.01" name="reports[{{ $index }}][accb]" id="accb_{{ $index }}" value="{{ $report->accb }}">
                                      <input  class="hidden-field" type="text" step="0.01" name="reports[{{ $index }}][institution_number]" id="institution_number_{{ $index }}" value="{{ $report->institution_number }}">
                                        <input  class="hidden-field" type="number" step="0.01" name="reports[{{ $index }}][transit_number]" id="transit_number_{{ $index }}" value="{{ $report->transit_number }}">
                                          <input  class="hidden-field" type="number" step="0.01" name="reports[{{ $index }}][account_number]" id="account_number_{{ $index }}" value="{{ $report->account_number }}">
        ...
        selects.forEach(select => {
            select.addEventListener('change', function () {
                ...
                document.getElementById(`accb_${index}`).value = accb.toFixed(2);
                 document.getElementById(`institution_number{index}`).value = institution_number.toFixed(2);
                  document.getElementById(`transit_number{index}`).value = transit_number.toFixed(2);
                   document.getElementById(`account_number{index}`).value = account_number.toFixed(2);
