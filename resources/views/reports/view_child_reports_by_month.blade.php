...
@section('content')
    ...
    <div class="container-fluid py-4">
        ...
        <!-- FILTER FORM -->
        <div class="mb-3">
            <form method="GET" action="{{ route('center.month.reports.view', [$center->center_id, $report_month]) }}"
                class="row g-2">
                    ...
                    <table class="table table-hover table-bordered align-middle mb-0">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th>
                                    <a
                                        href="?sort_by=child_name&sort_dir={{ request('sort_by') === 'child_name' && request('sort_dir') === 'asc' ? 'desc' : 'asc' }}">
                                        Child Name
                                ...
                                <th>
                                    <a
                                        href="?sort_by=parent&sort_dir={{ request('sort_by') === 'parent' && request('sort_dir') === 'asc' ? 'desc' : 'asc' }}">
                                        Parent
                                ...
                                <th>
                                    <a
                                        href="?sort_by=class&sort_dir={{ request('sort_by') === 'class' && request('sort_dir') === 'asc' ? 'desc' : 'asc' }}">
                                        Class
                                ...
                                <th>
                                    <a
                                        href="?sort_by=fee&sort_dir={{ request('sort_by') === 'fee' && request('sort_dir') === 'asc' ? 'desc' : 'asc' }}">
                                        Fee Plan
                        ...
                        <tbody>
                            ...
                            @foreach($children as $report)
                                ...
                                <tr>
                                    ...
                                    <td>{{ $report->child->child_first_name ?? 'N/A' }}
                                        {{ $report->child->child_last_name ?? 'N/A' }}</td>
                                    <td>{{ $report->child->parent_first_name ?? 'N/A' }}
                                        {{ $report->child->parent_last_name ?? 'N/A' }}</td>
